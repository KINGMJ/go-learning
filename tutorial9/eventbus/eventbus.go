...
func (bus *EventBus) Publish(topic string, args ...interface{}) error {
	...
	if handlers, ok := bus.handlers[topic]; ok && 0 < len(handlers) {
		...
		for i, handler := range copyHandlers {
			...
			if !handler.async {
				// 对于同步事件，如果订阅者其中一个执行错误，就直接退出
				err := bus.doPublish(handler, args...)
				if err != nil {
					return err
				}
			} else {
...
// 支持返回错误
func (bus *EventBus) doPublish(handler *eventHandler, args ...interface{}) error {
	passedArguments := bus.setUpPublish(handler, args...)
	// 函数调用
	errs := handler.callBack.Call(passedArguments)
	// 判断是否出现错误
	var resError error
	if len(errs) > 0 {
...
func (bus *EventBus) doPublishAsync(handler *eventHandler, args ...interface{}) error {
	defer bus.wg.Done()
	if handler.transactional {
		defer handler.Unlock()
	}
	return bus.doPublish(handler, args...)
}
